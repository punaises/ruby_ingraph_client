# encoding: utf-8

module IngraphRB
  # interface to fetch performance data
  class PerformanceData
    def self.plot_id(host_name, plot_name, db)
      sql = <<-SQL
        select plot.id from plot
        inner join hostservice on plot.hostservice_id = hostservice.id
        inner join host on hostservice.host_id = host.id
        where plot.name = '?' and host.name = '?'
      SQL
      db.fetch(sql, host_name, plot_name).first[:id]
    end

    def self.sql_for_time_limit(limit)
      return '' unless limit

      case limit
      when String
        " AND to_timestamp(timestamp) > now() - interval '#{limit}'"
      when Array
        " AND to_timestamp(timestamp) >= now() - interval '#{limit[0]}'" +
          " AND to_timestamp(timestamp) < now() - interval '#{limit[1]}'"
      end
    end

    attr_reader :plot_id, :host_name, :plot_name

    def initialize(db, host_name, plot_name, opts = {})
      @db = db
      @host_name = host_name
      @plot_name = plot_name
      @timeone = opts[:timezone]
      @limit = opts[:limit]
      @offset = opts[:offset] || 0
      @x = opts[:x_key] || :x
      @y = opts[:y_key] || :y
      @plot_id = self.class.plot_id(host, description, db)
    end

    def limit_to(span)
      @limit = span
      self
    end

    def sql
      sql = ''
      sql << "SET TIME ZONE '#{@timezone}';" if @timezone
      sql << "SELECT (timestamp + #{@offset}) as time, timetamp, min as value"
      sql << "FROM datapoint WHERE plot_id = #{@plot_id}"
      sql << self.class.sql_for_time_limit(@limit)
      sql << ' ORDER BY timestamp ASC'
      sql
    end

    def to_timeseries
      rows = db.fetch(sql).all
      if block_given?
        yield rows
      else
        rows
      end
    end
  end
end
